...
{
	zassert_ok(rb_push(20),"Insert a value to test");
	zassert_ok(rb_init(4),"Reinicialization of buffer");
...
{
	int v;
	zassert_ok(rb_push(42), "rb_push failed");
	zassert_ok(rb_pop(&v), "rb_pop failed");
...
{
	int v;
	zassert_ok(rb_push(1),"Insert first value failed");
	zassert_ok(rb_push(2),"Insert second value failed");
	zassert_ok(rb_push(3),"Insert third value failed");
	zassert_ok(rb_pop(&v), "rb_pop first value failed");
	zassert_equal(v, 1, "Popped value should be 1");
	zassert_ok(rb_pop(&v), "rb_pop second value failed");
	zassert_equal(v, 2, "Popped value should be 2");
	zassert_ok(rb_pop(&v), "rb_pop third value failed");
	zassert_equal(v, 3, "Popped value should be 3");
...
ZTEST(ring_buf_push_pop, test_push_full_returns_enospc)
{
	...
	zassert_ok(rb_push(1),"Insert first value failed");
	zassert_ok(rb_push(1),"Insert first value failed");
	zassert_equal(rb_push(1), -ENOSPC,"full buffer return -EINVAL");
